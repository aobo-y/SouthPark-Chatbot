...
def train():
    ...
    model = ChatBotModel(False, config.BATCH_SIZE, use_lstm=config.USE_LSTM)
    model.build_graph()
...
def chat():
    ...
    model = ChatBotModel(True, batch_size=1, use_lstm=config.USE_LSTM)
    model.build_graph()
...
class Chatbot(object):
    ...
    def response(self, line):
        ...
        if (len(token_ids) > self.max_length):
            raise RuntimeError('Max length the bot can handle is:', self.max_length)
